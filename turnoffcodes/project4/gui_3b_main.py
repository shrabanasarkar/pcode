...
layout = [[sg.Text('Capacitor Assisted Zero Voltage Switching (ZVS) Loss Calculator for SiC MOSFET in Half Bridge Configuration', auto_size_text=False,
          ...
          [sg.Text('DC bus voltage (Vdc)', font=(30), size=(20, 1)),
           ...
           sg.Text('(A)', font=(30), size=(3, 1))],
            [sg.Text('External capacitance(Cext)', font=(30), size=(25, 1)),
           sg.InputText(font=(30), do_not_clear=True,key = 'Cext',justification='left', size=(5, 1)),
           sg.Text('(pF)', font=(30), size=(3, 1)), sg.Text('', size=(2, 1)),sg.Text(key='OUT5', font=(30), size=(50, 1)),],
          ...
          [sg.Text('Ld', font=(30), size=(12, 1)),
           ...
           sg.Text('(nH)', font=(30), size=(5, 1)), sg.Text('', size=(5, 1)),sg.Text('Ldc', font=(30), size=(12, 1)),
           sg.InputText(font=(30), do_not_clear=True,key='Ldc', justification='left', size=(5, 1)),
    ...
    if  event=='Submit':
        ...
        if b1 != '':
            ...
            if float(values['Ld']) <= 0:
                ...
                b = 1
            if float(values['Ldc']) <= 0:
                sg.popup("Enter correct value of Ldc")
                b = 1
                ...
                if (r_gext + r_gint) <= (2 * l_s / c_iss) ** .5:
                    ...
                    for ii in np.arange(i_0_min, i_0_max + 1, i_0_step):
                        ...
                        while (i < n):
                            ...
                            vgs.append(vgs[i] + tau * ((
                                                               v_ee + ((coxd * r_g / (coxd + cds)) * (ii - ich[i])) -
                                                               vgs[i] - ((vdpsp[i] - vds[i]) * l_s / (
                                                               l_d + l_s))) / ((r_g * (c_gs + coxd)) - (
                                        r_g * (coxd ** 2) / (coxd + cds)))))
                            vds.append(
                    ...
                    #     # ----------------------------------submode C--------------------------------------------
                        ...
                        while i < n:
                            ...
                            vgs.append(vgs[i - 1] + (tau / time_1) * (
                                        v_ee - vgs[i] + time_2 * (idc[i] - ich[i]) / (coss + c_gd_ext + c_ext)))
                        ...
                        if Flag3 == 1:
                            window_first['OUT5'].update('Soft switching not satisfied,increase the value of Cext')
                            break
                        plt.figure(5)
                        plt.subplot(2, 3, 1)
                        plt.plot(t, vgs, marker='o', ms=3, color='black', label="Vgs vs t")
                        ...
                        def eqn_cq1(vds):
                            ...
                            # print((cteq * cbeq) / (cteq + cbeq))
                            # return ( (cteq + cbeq)/(cteq * cbeq))
                            return ((cteq * cbeq) / (v_dc * (cteq + cbeq)))
                        ...
                        if Flag3 == 0:
                            ...
                            A1 = ii / cq2
                            A2 = (vpdpsp - A1) / wo
                            cbeqconj = (k4 / ((1 + (v_dc / k5)) ** m2)) + c_ext + c_gd_ext
                            A3 = ldc * cbeqconj * (wo ** 2) - 1
                            TIII = (v_dc - Ao) / A1
                            vdpsp3 = Ao + (A1 * TIII) + (A2 * math.sin(wo * TIII))
                            # vdpsp3 = v_dc - Ao - (A1 * TIII) + (A2 *A3* math.sin(wo * TIII))
