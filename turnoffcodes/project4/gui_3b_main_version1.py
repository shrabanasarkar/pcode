...
for ii in np.arange(20, 21, i_0_step):
    ...
    while i < n:
        ...
        vgs.append(vgs[i-1] + (tau/time_1) * (v_ee -vgs[i] + time_2*(idc[i] - ich[i])/(coss + c_gd_ext + c_ext)))
    ...
    while i < n and (Flag2 == 0):
        if (vgs[i] > vth):
            ...
            vds.append(vds[i] + tau * ((idc[i] - ich[i]) / (coss + c_gd_ext + c_ext)))
            vgs.append(
                vgs[i - 1] + (tau / time_1) * (v_ee - vgs[i] + time_2 * (idc[i] - ich[i]) / (coss + c_gd_ext + c_ext)))
    ...
    #     vd = v_dc - vds
        ...
        # cbeq = np.dot(coss ,( c_ext + c_gd_ext))
        # int1a = np.dot(cteq, cbeq)
        # int1 = np.dot(int1a ,np.power(np.dot(v_dc,np.add(cteq ,cbeq)),-1))
