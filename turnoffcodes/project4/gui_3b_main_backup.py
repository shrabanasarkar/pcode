...
for ii in np.arange(20, 21, i_0_step):
    ...
    def funcr(v_dc,vds):
        vd = np.subtract(v_dc,vds)
        coss = np.dot(k4,np.power(np.add(1,np.dot(1/k5,vds)),(-m2)))
        cd = np.dot(k4,np.power(np.add(1,np.dot(1/k5,vd)),(-m2)))
        cteq = np.add(cd ,( c_ext + c_gd_ext))
        cbeq = np.add(coss ,( c_ext + c_gd_ext))
        int1a = np.dot(cteq, cbeq)
        int1 = np.dot(int1a ,np.power(np.add(cteq ,cbeq),-1))
        y1 =  np.dot(np.power(int1a,-1) ,np.add(cteq ,cbeq))
        y2 = np.add(cteq ,cbeq)
        sum1 = 0
        for p in range(0,len(vds)):
            sum1 = sum1 + (int1[p]*.1)
        print("vds = ", vds)
        print("vd = ", vd)
        print("coss = ", coss)
    ...
    # plt.figure(6)
    # plt.subplot(2, 3, 1)
    # plt.plot(vds, y11, marker='o', ms=3, color='black')
    # plt.xlabel('vds')
    # plt.ylabel('cteq // cbeq')
    # plt.subplot(2, 3, 2)
    # plt.plot(vds, y22, marker='o', ms=3, color='black')
    # plt.xlabel('vds')
    # plt.ylabel('cteq + cbeq')
    plt.show()
