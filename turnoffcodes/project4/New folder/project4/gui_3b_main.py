            ...
            if b == 2:
                ...
                if (r_gext + r_gint) <= (2 * l_s / c_iss) ** .5:
                    ...
                    for ii in np.arange(i_0_min, i_0_max + 1, i_0_step):
                        ...
                        T1A = (i - 1) * tau
                        # sampl = (T1A - 0) / tau
                        # t = np.linspace(0, T1A, (int)(sampl - 1))
                        print("Io=",ii,"T1A =",T1A,"i=" ,i,"len(vgs)=", len(vgs), "len(vds)=",len(vds), "len(id)=",len(id), "len(vdpsp)=",len(vdpsp))
                    ...
                    # ----------------------------------------------submode B-------------------
                        ...
                        while (j < n):
                            ...
                            vdpsp.append(vdpsp[j] + tau * (ii - id[j]) / c_gd_ext)
                            id.append(id[j] + (tau / (l_d + l_s)) * (vdpsp[j] - vds[j]))

                            vgs.append(vgs[j] + tau * (v_ee + (cgd / (cgd + cds)) * r_g * (ii - ich[j]) - vgs[j] - (
                                    vdpsp[j] - vds[j]) * l_s / (l_d + l_s)) / (
                                                   r_g * (c_gs + cgd) - r_g * (cgd ** 2) / (cgd + cds)))

                            vds.append(vds[j] + tau * ((ii - ich[j]) + (cgd / (r_g * (c_gs + cgd))) * (
                                        (v_ee - vgs[j] - (l_s * (vdpsp[j] - vds[j])
                                                          / (l_d + l_s))) / ((cgd + cds) - (cgd ** 2) / (c_gs + cgd)))))
                            s = vds[j] * ich[j] * tau
                            E1B = E1B + s
                            j = j + 1
                        Vgs1B = vgs[j]
                    ...
                    #     else:
                    #         Ich3A = 0
                    #     cosseq3 = (k4 / (1 + (v_dc / k5)) ** m2) + c_gd_ext
                    ...
                    #
                    # if event == 'Submit':
                    #     otpt = pd.DataFrame(list(zip(*[i_0, t_off, e_off, dv_by_dt, di_by_dt]))).add_prefix('Col')
                    #     otpt.to_csv('output_orig_a.csv', mode='w', index=False,
                    #                 header=['I_0(A)', 'T_off(ns)', 'E_off(uJ)', 'dv/dt(V/ns)', 'di/dt(A/ns)'])
                    #
                    # plt.figure(2)
                    # plt.subplot(2, 3, 1)
