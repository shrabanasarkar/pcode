...
def func_cgd_vdg_low(vdg, k1a, m1a, k2a):
    return k1a / (((1 + (vdg / k2a)) ** m1a) )
...
def func_cd_vd(vd, k6, k7, m3):
    return k6 / ((1 + vd / k7) ** (m3))
...
def reverse_transfer_capacitance(file_rtfr):
    ...
    while (count_value <len(vds)):
        # print("vds=",vds1.get(count_value),vds1[count_value])
        if (float(vds1.get(count_value)) >= .1):
            ...
            # cgd_vds3.append(math.log(cgd_vds2.get(count_value)))
        count_value += 1
        # otpt = pd.DataFrame(list(zip(*[vds3,cgd_vds3]))).add_prefix('Col')
        # otpt.to_csv('output_1new.csv', mode='w', index=False,
        #             header=['vds3', 'cgd_vds3'])

    # plt.figure(15)
    # plt.plot(vds3, cgd_vds3, marker='o', ms=3, color='black', label="Cgd_vds data")
    #
    # plt.xlabel('Vds(V)')
...
#---------------------------
    ...
    params, _ = curve_fit(func_cgd_vdg_low, np.transpose(vdg1), np.transpose(cgd1),
                          bounds=((0, 0,0), (np.inf, np.inf, np.inf)),
                          method='trf', maxfev=20000)
    k1a, m1a, k2a = params[0], params[1], params[2]
    print('k1a',k1a,'m1a', m1a,'k2a' ,k2a)

    cgd_vds_fit = k1a / ((1 + (vdg1 / k2a)) ** m1a )
    ...
    plt.plot(vdg1, cgd_vds_fit, label="Cgd_vds = k1a / ((1 + (vdg1 / k2a)) ** m1a )")
    ...
    params, _ = curve_fit(func_cgd_vdg_high, np.transpose(vdg2), np.transpose(cgd2),bounds=((0, 0,0), (np.inf, np.inf, np.inf)),
    ...
    return k1b, k2b, m1b, k1a, k2a, m1a,vtd
...
def diode_capacitance(file_dcap):
    vd = pd.read_csv(file_dcap, header=None, usecols=[0])
    cd_vd = pd.read_csv(file_dcap, header=None, usecols=[1])
    vd = vd.squeeze()
    cd_vd = cd_vd.squeeze()
    vd = vd.to_numpy()
    cd_vd = (cd_vd.to_numpy()) / 1000
    params_cd_vd, _ = curve_fit(func_cd_vd, np.transpose(vd), np.transpose(cd_vd))
    k6, k7, m3 = params_cd_vd[0], params_cd_vd[1], params_cd_vd[2]
    cd_vd_fit = k6 / ((1 + vd / k7) ** (m3))

    plt.subplot(2, 3, 3)
    plt.plot(vd, cd_vd, marker='o', ms=3, color='black', label="Cd_vd data")
    plt.plot(vd, cd_vd_fit, label="Cd_vd = k6/((1+vd/k7)**(m3))")
    plt.xlabel('Vd(V)')
    plt.ylabel('Cd_vd(nF)')
    plt.legend(loc='best', fancybox=True, shadow=True)
    plt.yscale('log')
